...
export const EditarPerfilPage = () => {
  ...
  const cargarPerfil = async () => {
    try {
      ...
      const { informacionPersonal, informacionBancaria, metadatos } = response.data;
  ...
  const ejecutarActualizacionBancaria = async (codigoMFA) => {
    try {
      ...
      setErrorBancaria(error.message || 'Error al actualizar información bancaria');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a1a2e] to-[#16213e]">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {perfil?.metadatos?.ultima_modificacion && (
          <div className="mb-6 p-4 bg-yellow-500/20 border border-yellow-500/50 rounded">
            <p className="text-yellow-400 text-sm">
              ℹ️ Última modificación: {new Date(perfil.metadatos.ultima_modificacion).toLocaleDateString('es-ES')}
              <br />
        ...
        <div className="bg-[#1e2a38] rounded-lg border border-[#2a3f5f] p-6 mb-6">
          ...
          <form onSubmit={handleSubmitPersonal}>
            ...
            >
              {loadingPersonal ? 'Guardando...' : '🔒 Guardar Cambios (Requiere MFA)'}
            </button>
          </form>
        </div>
        ...
        <div className="bg-[#1e2a38] rounded-lg border border-[#2a3f5f] p-6">
          ...
          <form onSubmit={handleSubmitBancaria}>
            ...
            <button
              ...
              className="mt-6 w-full px-4 py-3 bg-[#66c0f4] text-white font-medium rounded hover:bg-[#5ab0e4] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {loadingBancaria ? 'Actualizando...' : '🔒 Actualizar Información Bancaria (Requiere MFA)'}
            </button>
          </form>
        </div>
      </main>
      ...
      <MFAVerificationModal
        ...
        title={mfaAction === 'personal' ? 'Verificar MFA - Información Personal' : 'Verificar MFA - Información Bancaria'}
      />
    </div>
  );
};
